...
// holds the St. Joseph's Placements and Training Cell tokens in its own httpOnly cookie.
const nodes = [
...
export default function AuthFlowDiagram() {
  return (
    <FlowDiagram
      nodes={nodes}
      caption="The browser only ever holds your app's own session cookie — never the St. Joseph's Placements and Training Cell tokens. To refresh, your server re-sends the stored refresh token to GET /auth/refresh as a Cookie: LH_refresh=… header (the endpoint reads it only from that cookie)."
    />
  )
}
