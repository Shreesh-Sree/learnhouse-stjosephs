...
Keeping St. Joseph's Placements and Training Cell up to date ensures you have the latest features, bug fixes, and security patches.

## Updating St. Joseph's Placements and Training Cell
...
St. Joseph's Placements and Training Cell uses [Alembic](https://alembic.sqlalchemy.org/) to evolve the database schema between versions. After the new container starts, the CLI prompts:
...
Early St. Joseph's Placements and Training Cell installs created their schema via SQLAlchemy's `create_all` without Alembic tracking. The update command detects this automatically and stamps the current schema as the baseline before running any delta migrations — so `upgrade head` applies only new changes instead of replaying everything.
...
Review the [St. Joseph's Placements and Training Cell release notes](https://github.com/learnhouse/learnhouse/releases) before updating to understand what has changed. Some updates may include:
