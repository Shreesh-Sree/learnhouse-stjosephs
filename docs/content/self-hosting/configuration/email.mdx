...
St. Joseph's Placements and Training Cell supports two email providers for transactional email delivery: **Resend** and **SMTP**. Email is used for user invitations, password resets, and system notifications.
...
Emails arrive as `St. Joseph's Placements and Training Cell <noreply@example.com>`. `LEARNHOUSE_SYSTEM_EMAIL_SENDER_NAME` replaces the `St. Joseph's Placements and Training Cell` part for the whole deployment:

```env
LEARNHOUSE_SYSTEM_EMAIL_SENDER_NAME=Acme Academy
```

It defaults to `St. Joseph's Placements and Training Cell`, so leaving it unset changes nothing.
...
St. Joseph's Placements and Training Cell sends the following types of email:
