...
This page is the deployment reference for St. Joseph's Placements and Training Cell environment variables. The backend
...
| --- | --- | --- | --- | --- |
| `LEARNHOUSE_SITE_NAME` | Instance name returned by the API. | string | `St. Joseph's Placements and Training Cell` | API |
| `LEARNHOUSE_SITE_DESCRIPTION` | Short instance description returned by the API. | string | `St. Joseph's Placements and Training Cell is an open-source platform tailored for learning experiences.` | API |
...
| `LEARNHOUSE_PLATFORM_URL` | Base URL for St. Joseph's Placements and Training Cell platform links, upgrade URLs, and email allowlist checks. | URL | unset; backend flows fall back to `https://www.learnhouse.app` and the web token-exchange to `https://learnhouse.app` | API, Web |
...
| `LEARNHOUSE_SYSTEM_EMAIL_SENDER_NAME` | Display name on system email, as in `Name <address>`. The address itself is not configurable per sender. | string | `St. Joseph's Placements and Training Cell` | API |
