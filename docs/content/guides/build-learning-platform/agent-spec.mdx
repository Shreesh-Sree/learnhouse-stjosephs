---
title: Agent Spec
description: A self-contained, copy-paste specification a coding agent can follow to build a headless St. Joseph's Placements and Training Cell learning platform with Next.js, including the exact API contract and acceptance criteria.
---
...
Copy everything in the block below, fill in the **Configuration** values at the top, and paste it into your coding agent (see [Let an Agent Build It](/guides/build-learning-platform/with-an-agent) for the workflow). It's written *for the model* and contains the real St. Joseph's Placements and Training Cell API contract, so the agent builds against the API that actually exists.
...
````markdown
# SPEC: Build a headless learning platform on the St. Joseph's Placements and Training Cell API

## Configuration (fill these in before starting)
- API_BASE = http://localhost:1338/api/v1     # your St. Joseph's Placements and Training Cell API base URL
...
that reads courses and content from the St. Joseph's Placements and Training Cell REST API and supports user
...
## Hard rules
- St. Joseph's Placements and Training Cell access/refresh tokens MUST NEVER reach the browser. Do all
