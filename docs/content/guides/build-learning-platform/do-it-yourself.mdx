...
description: A complete, tested, step-by-step guide to building a headless learning platform with Next.js and the St. Joseph's Placements and Training Cell API — public catalogue first, then authentication, enrollment, and progress tracking.
---
...
This is the full build. By the end you'll have a Next.js app that browses your courses, plays their content, signs learners in, enrolls them, and tracks progress — all on top of the St. Joseph's Placements and Training Cell API.
...
<Callout type="info">
**Prerequisites** (from the [overview](/guides/build-learning-platform)): Node 18+, a running St. Joseph's Placements and Training Cell instance at `http://localhost:1338`, your **org slug**, and at least one **public, published course** with content. Everything reads the API base from one env var, so adjust it if your instance is elsewhere.
</Callout>
...
Every call to St. Joseph's Placements and Training Cell goes through one small helper. Create `src/lib/learnhouse.ts`. This mirrors the pattern the official frontend uses in `apps/web/services/utils/ts/requests.ts` — a thin `fetch` wrapper that surfaces the API's `{ "detail": "..." }` error shape.
...
/**
 * Fetch JSON from the St. Joseph's Placements and Training Cell API.
...
export async function lhFetch<T = any>(
  ...
  if (!res.ok) {
    ...
    throw new Error(`St. Joseph's Placements and Training Cell API ${res.status}: ${detail}`)
  }
  return res.json()
}
...
Open [http://localhost:3000](http://localhost:3000) and you'll see your courses. (If St. Joseph's Placements and Training Cell is also on `:3000`, run the API on its usual port and Next will pick `:3001` — or set `PORT`.)
...
Now we add accounts. The golden rule: **St. Joseph's Placements and Training Cell tokens never reach the browser.** Your Next.js app logs in on the server, stores the tokens in its *own* httpOnly cookie, and attaches them when calling the API. This is the Backend-for-Frontend (BFF) pattern.
...
Create `src/lib/session.ts` to read and write the session cookie. It stores the St. Joseph's Placements and Training Cell tokens, encrypted-at-rest concerns aside, in an httpOnly cookie that the browser can't read from JavaScript.
...
export async function authedFetch<T = any>(
  ...
  if (!res.ok) throw new Error(`St. Joseph's Placements and Training Cell API ${res.status}`)
  return res.json()
}
```
