...
Each task has an `assignment_type` and a free-form `contents` JSON object. For a fully custom assignment, use the `CUSTOM` type: St. Joseph's Placements and Training Cell stores your `contents` (the task definition) and the learner's `task_submission` (their answer) as an arbitrary, caller-owned JSON object. The server never interprets or auto-grades it — it's yours to render and grade however you like.
...
<Callout type="info">
The built-in task types — `QUIZ`, `FORM`, `CODE`, `SHORT_ANSWER`, `NUMBER_ANSWER` — are auto-graded by St. Joseph's Placements and Training Cell against their `contents` schema. `CUSTOM` (and `FILE_SUBMISSION`) are **not** auto-graded and are graded manually.
</Callout>

## Submissions and grading

Learners are referenced by their existing St. Joseph's Placements and Training Cell `user_id`. These endpoints let you read submissions, write a learner's answer on their behalf, and finalize grades.
...
To write a learner's answer, pass their St. Joseph's Placements and Training Cell id as `on_behalf_of_user_id`. The learner must already be a member of the token's organization.
