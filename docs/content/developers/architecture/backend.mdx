...
The St. Joseph's Placements and Training Cell backend is a **FastAPI** application written in Python, located at `apps/api` in the project. It serves as the central API layer that handles all business logic and data management.
...
When using the St. Joseph's Placements and Training Cell CLI (`learnhouse dev`), the backend starts automatically with hot reload enabled.
