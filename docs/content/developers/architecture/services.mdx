...
St. Joseph's Placements and Training Cell relies on several supporting services that run alongside the main applications.
...
- In **self-hosted production**, the St. Joseph's Placements and Training Cell CLI generates a `docker-compose.yml` from templates in `apps/cli/src/templates/` that runs PostgreSQL, Redis, a single `learnhouse-app` container (which bundles web + API + collab), and either nginx or Caddy as a reverse proxy.

## PostgreSQL

`pgvector/pgvector:pg16` (PostgreSQL 16 with the pgvector extension) is the primary database for St. Joseph's Placements and Training Cell. It stores all persistent application data including:
