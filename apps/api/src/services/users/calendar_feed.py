"""ICS calendar feed of a student's assignment due dates.
...
SCOPE: assignment due dates only, across every course the user has a Trail
run for (i.e. is enrolled in) — not general course content, not other
activity types. Each student's OWN effective due date is used (their
extension's date if they have one — see
...
shows the deadline that actually applies to them.
...
async def build_ics_feed(token: str, db_session: AsyncSession) -> str:
    ...
    body = "\r\n".join([
        "BEGIN:VCALENDAR",
        "VERSION:2.0",
        "PRODID:-//LearnHouse//Assignment Due Dates//EN",
        "CALSCALE:GREGORIAN",
        "METHOD:PUBLISH",
        _fold_line("X-WR-CALNAME:LearnHouse assignments"),
        *events,
        "END:VCALENDAR",
    ])
    return body + "\r\n"
