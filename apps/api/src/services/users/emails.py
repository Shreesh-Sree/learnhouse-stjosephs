...
# Text wordmark shown in the header of every transactional email. Plain
# styled text rather than an image — reliable across every mail client
# (some strip inline SVG/large embedded images entirely, e.g. Gmail) and
# avoids bloating every email with an embedded raster logo.
LOGO_SVG = (
    '<div style="font-size:14px;font-weight:900;letter-spacing:0.02em;'
    'color:#000000;">ST. JOSEPH\'S PLACEMENTS AND TRAINING CELL</div>'
)
...
def _org_logo_img(logo_url: str, alt: str) -> str:
    ...
    Bounded to the same footprint as the St. Joseph's Placements and Training Cell wordmark. Raster logos
    ...
    org-branded, never a broken St. Joseph's Placements and Training Cell mark.
    """
    return (
...
def _email_layout(
    ...
    ``logo_html`` defaults to the St. Joseph's Placements and Training Cell mark; white-labeled emails pass the
...
def send_account_creation_email(
    ...
    the subject and body name the org (not St. Joseph's Placements and Training Cell), the org's ``logo_url``
    replaces the St. Joseph's Placements and Training Cell mark when present, and the footer is reduced to a
    subtle "Powered by St. Joseph's Placements and Training Cell". Org-less signups keep the St. Joseph's Placements and Training Cell-branded
    ...
    cta_button = (
        f'<a href="{html.escape(cta_url)}" style="{STYLES["button"]}">{cta}</a>'
        if cta_url else ""
    )
    body_content = f"""
...
def send_org_join_email(
    ...
    academy, not onto St. Joseph's Placements and Training Cell — with the org's logo when it has one.
    """
