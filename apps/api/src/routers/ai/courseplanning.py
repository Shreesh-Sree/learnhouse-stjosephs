...
async def event_generator(generator, session_uuid: str):
    """Convert async generator to SSE format with immediate flush and keepalive."""
    try:
        # Send immediate comment to flush headers and establish HTTP/3 / QUIC stream
        yield ": connected\n\n"
        gen_iter = generator.__aiter__()
        done = False
        while not done:
            try:
                chunk = await asyncio.wait_for(gen_iter.__anext__(), timeout=10.0)
                yield f"data: {json.dumps({'type': 'chunk', 'content': chunk})}\n\n"
            except asyncio.TimeoutError:
                # Keep-alive to prevent proxy / Cloudflare QUIC stream timeout
                yield ": ping\n\n"
...
def sanitize_prosemirror_doc(node: any, parent_type: str = "root") -> any:
    ...
    if isinstance(node, (str, int, float)):
        text_str = str(node)
        if parent_type in ("bulletList", "orderedList"):
            return {
                "type": "listItem",
                "content": [
                    {
    ...
    # Infer missing type
    if not clean_node.get("type"):
        if "text" in clean_node:
            clean_node["type"] = "text"
        elif "question" in clean_node and "answer" in clean_node:
            clean_node["type"] = "flipcard"
            attrs = clean_node.get("attrs", {}) or {}
            attrs.setdefault("question", clean_node.pop("question"))
            attrs.setdefault("answer", clean_node.pop("answer"))
            attrs.setdefault("color", clean_node.pop("color", "blue"))
            attrs.setdefault("alignment", clean_node.pop("alignment", "center"))
            attrs.setdefault("size", clean_node.pop("size", "medium"))
            clean_node["attrs"] = attrs
        elif parent_type in ("bulletList", "orderedList"):
            clean_node["type"] = "listItem"
        elif parent_type in ("listItem", "tableCell", "tableHeader") or "content" in clean_node:
            clean_node["type"] = "paragraph"
        else:
            clean_node["type"] = "paragraph"

    node_type = clean_node.get("type")

    # If text node, normalize text and marks
    if node_type == "text":
        clean_node["text"] = str(clean_node.get("text", "") or "")
        marks = clean_node.get("marks")
        if isinstance(marks, list):
            cleaned_marks = []
            for mark in marks:
                if isinstance(mark, str):
                    m_type = "bold" if mark.lower() == "strong" else ("italic" if mark.lower() == "em" else mark)
                    cleaned_marks.append({"type": m_type})
                elif isinstance(mark, dict):
                    m_type = mark.get("type", "")
                    if m_type.lower() == "strong":
                        m_type = "bold"
                    elif m_type.lower() == "em":
                        m_type = "italic"
                    cleaned_marks.append({**mark, "type": m_type})
            clean_node["marks"] = cleaned_marks
        clean_node.pop("content", None)
        return clean_node

    # If listItem, children must be blocks (like paragraph)
    if node_type == "listItem":
        content = clean_node.get("content")
        if not isinstance(content, list) or not content:
            clean_node["content"] = [{"type": "paragraph", "content": []}]
        else:
            sanitized_content = []
            for child in content:
                if isinstance(child, (str, int, float)):
                    sanitized_content.append({
                        "type": "paragraph",
                        "content": [{"type": "text", "text": str(child)}]
                    })
                elif isinstance(child, dict) and child.get("type") == "text":
                    sanitized_content.append({
                        "type": "paragraph",
                        "content": [sanitize_prosemirror_doc(child, "paragraph")]
                    })
                else:
                    sc = sanitize_prosemirror_doc(child, "listItem")
                    if sc:
                        sanitized_content.append(sc)
            clean_node["content"] = sanitized_content
        return clean_node

    # If node has content array
    if "content" in clean_node and isinstance(clean_node["content"], list):
        sanitized_content = []
        for child in clean_node["content"]:
            sc = sanitize_prosemirror_doc(child, node_type)
            if sc:
                sanitized_content.append(sc)
        clean_node["content"] = sanitized_content
...
async def save_activity_content(
    ...
    # Content MUST be a ProseMirror document object.
    if not isinstance(content, dict):
    ...
    # Automatically sanitize and normalize ProseMirror nodes before validation
    content = sanitize_prosemirror_doc(content, "root")
