...
export async function startActivityAIChatSession(
  ...
  if (byok?.apiKey) {
    data.byok_api_key = byok.apiKey
    data.byok_provider = byok.provider
    data.byok_model = byok.model
  }
  const customHeaders: Record<string, string> = byok?.apiKey
    ? {
        'X-BYOK-Api-Key': byok.apiKey,
        ...(byok.provider ? { 'X-BYOK-Provider': byok.provider } : {}),
        ...(byok.model ? { 'X-BYOK-Model': byok.model } : {}),
      }
    : {}

  const result = await fetch(
    `${getAPIUrl()}ai/start/activity_chat_session`,
    RequestBodyWithAuthHeader('POST', data, customHeaders, access_token)
  )
