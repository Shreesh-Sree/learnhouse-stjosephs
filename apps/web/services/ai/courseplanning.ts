...
/**
 * Recursively sanitize and repair ProseMirror document nodes generated by AI.
 * Ensures:
 * - Root is { type: "doc", content: [...] }
 * - Every node has a valid 'type' field
 * - Bare strings/numbers are converted to { type: "text", text: "..." }
 * - Bare text objects {"text": "..."} missing "type" get type="text"
 * - Objects with "content" missing "type" get type="paragraph"
 * - Mark types "strong" -> "bold", "em" -> "italic"
 * - Lists only contain listItem nodes, and listItems wrap text in paragraphs
 * - Flipcards without type get type="flipcard"
 */
export function sanitizeProseMirrorDoc(node: any, parentType: string = 'root'): any {
  if (node === null || node === undefined) return null

  // If node is a primitive string or number, convert to appropriate node
  ...
  // Clone object
  const cleanNode: any = Array.isArray(node) ? node : { ...node }

  // If array passed at root, wrap into doc
  if (Array.isArray(cleanNode)) {
    return {
      type: 'doc',
      content: cleanNode.map((c) => sanitizeProseMirrorDoc(c, 'doc')).filter(Boolean),
    }
  }

  // Infer missing type if not present
  if (!cleanNode.type) {
    if (typeof cleanNode.text === 'string' || cleanNode.text !== undefined) {
      cleanNode.type = 'text'
    } else if (cleanNode.question && cleanNode.answer) {
      cleanNode.type = 'flipcard'
      cleanNode.attrs = {
        question: cleanNode.question,
        answer: cleanNode.answer,
        color: cleanNode.color || 'blue',
        alignment: cleanNode.alignment || 'center',
        size: cleanNode.size || 'medium',
        ...cleanNode.attrs,
      }
      delete cleanNode.question
      delete cleanNode.answer
    } else if (parentType === 'bulletList' || parentType === 'orderedList') {
      cleanNode.type = 'listItem'
    } else if (parentType === 'listItem' || parentType === 'tableCell' || parentType === 'tableHeader') {
      cleanNode.type = 'paragraph'
    } else if (Array.isArray(cleanNode.content)) {
      cleanNode.type = 'paragraph'
    } else {
      cleanNode.type = 'paragraph'
    }
  ...
  // If type is 'text', normalize text and marks
  if (cleanNode.type === 'text') {
    cleanNode.text = cleanNode.text !== undefined && cleanNode.text !== null ? String(cleanNode.text) : ''
    if (Array.isArray(cleanNode.marks)) {
      cleanNode.marks = cleanNode.marks
        .map((mark: any) => {
          if (typeof mark === 'string') {
            const mType = mark.toLowerCase() === 'strong' ? 'bold' : mark.toLowerCase() === 'em' ? 'italic' : mark
            return { type: mType }
          }
          if (mark && typeof mark === 'object') {
            let mType = mark.type || ''
            if (mType.toLowerCase() === 'strong') mType = 'bold'
            if (mType.toLowerCase() === 'em') mType = 'italic'
            return { ...mark, type: mType }
          }
          return null
        })
        .filter(Boolean)
  ...
  // If node is listItem, its children must be block nodes (like paragraph)
  if (cleanNode.type === 'listItem') {
    if (!Array.isArray(cleanNode.content) || cleanNode.content.length === 0) {
      cleanNode.content = [{ type: 'paragraph', content: [] }]
    } else {
      cleanNode.content = cleanNode.content
        .map((child: any) => {
          ...
          if (child && child.type === 'text') {
            return {
              type: 'paragraph',
              content: [sanitizeProseMirrorDoc(child, 'paragraph')],
            }
          }
          return sanitizeProseMirrorDoc(child, 'listItem')
        })
        .filter(Boolean)
    }
    return cleanNode
  ...
  // If node has content array, sanitize children recursively
  if (Array.isArray(cleanNode.content)) {
    cleanNode.content = cleanNode.content
      .map((child: any) => sanitizeProseMirrorDoc(child, cleanNode.type))
      .filter(Boolean)
  }
...
export function parseActivityContentFromStream(streamContent: string): any | null {
  try {
    ...
    let parsed = repairAndParseJson<any>(streamContent)
    if (!parsed || typeof parsed !== 'object') {
      return null
    }

    // Ensure root doc structure
    if (parsed.type !== 'doc') {
      if (Array.isArray(parsed)) {
        parsed = { type: 'doc', content: parsed }
    ...
    // Sanitize and repair all nodes recursively
    parsed = sanitizeProseMirrorDoc(parsed, 'root')

    // Validate ProseMirror structure
    const validation = validateProseMirrorDoc(parsed)
    if (!validation.valid) {
      console.warn('[parseActivityContent] Minor validation issue after sanitization:', validation.error)
      // Attempt another pass if needed
      parsed = sanitizeProseMirrorDoc(parsed, 'root')
    }
